...
class Financial:
    ...
    def parse_yahoo_finance(self):
        url = f"https://finance.yahoo.com/quote/{self.ticker}/financials/?p={self.ticker}"  # Fixed URL
       # без хедера отвал 
        headers = {
            "User-Agent": (
        ...
        try:
            response = httpx.get(url, headers=headers, timeout=10, follow_redirects=True)  # Follow redirects
    ...
    def parse_financial_table(self, html_content):
        ...
        # working with tablebody on the page yahoo finance
        table_body = soup.find('div', class_='tableBody yf-9ft13')

        if not table_body:
            print("Таблица финансов не найдена в предоставленном HTML.")
            return None
